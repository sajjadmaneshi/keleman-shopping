...
export class BasketRepository extends DataService<BasketItemViewModel> {
  ...
  mergeCart(
    ...
    return this._http.post(`${this._getCartUrl}/merge`, dto) as Observable<
      HttpClientResult<MergeResultViewModel[]>
    >;
  }

  getBasketCheckout(): Observable<HttpClientResult<BasketCheckoutViewModel>> {
    return this._http.get(`${this._getCartUrl}/checkout`) as Observable<
      HttpClientResult<BasketCheckoutViewModel>
    >;
  }

  getCartCount(): Observable<HttpClientResult<number>> {
    return this._http.get(`${this._getCartUrl}/count`) as Observable<
  ...
  addToCart(dto: AddToCartDto): Observable<HttpClientResult<boolean>> {
    return this._http.post(`${this._getCartUrl}`, dto) as Observable<
      HttpClientResult<boolean>
    >;
  }

  updateCart(dto: UpdateBasketDto): Observable<HttpClientResult<boolean>> {
    return this._http.patch(`${this._getCartUrl}`, dto) as Observable<
      HttpClientResult<boolean>
    >;
  }
  removeFromCart(orderId: number): Observable<HttpClientResult<boolean>> {
    return this._http.delete(`${this._getCartUrl}/${orderId}`) as Observable<
      HttpClientResult<boolean>
    >;
  }

  getReport(factorId?: number): Observable<Blob> {
    return this._http.get(`${this._getCartUrl}/report/${factorId || ''}`, {
      responseType: 'blob',
    }) as Observable<Blob>;
  }
  ...
  getPackageDetails(packageId: number) {
    return this._http.get(
      `${this._getCartUrl}/packageDetail/${packageId}`
